...
namespace NRRD
{
	...
	template <int N>
	inline void swapBytes(void *v) {}

	/// swap 2 bytes (convert short endian)
	template <>
	inline void swapBytes<2>(void *v)
	{
		char *d = (char *)v;
		std::swap(d[0], d[1]);
	}
	...
	template <>
	inline void swapBytes<4>(void *v)
	{
		short *d = (short *)v;
		std::swap(d[0], d[1]);
		swapBytes<2>(d);
	...
	template <>
	inline void swapBytes<8>(void *v)
	{
		int *d = (int *)v;
		std::swap(d[0], d[1]);
		swapBytes<4>(d);
			  ...
			  std::map<std::string, std::string> hdr_fields = std::map<std::string, std::string>())
	{
		...
		for (auto it = hdr_keys.begin(); it != hdr_keys.end(); ++it)
			if (it->second.find('\n') == std::string::npos)
				...
				ignored++;
		if (ignored)
			std::cerr << "NRRD::save<T>(...): Ignored " << ignored << " keys in meta info because of invalid characters (e.g. line breaks)\n";
...
#undef _DEFINE_TYPE
		...
		else
		{
			...
			*data = new T[n];
			...
			// Somewhere around here I need to do the decoding
